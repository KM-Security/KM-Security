...
KM-Security DoS tool ported to Go language from Python. 
...
The main difference from Python version layed in Golang architecture for concurrency: the goroutines. KM-Security.py runs
a new thread for each connection in the connection pool so it uses hundreds and thousands of threads. 
KM-Security.go just uses lightweight goroutines that used only tens of threads (commonly golang runtime started one thread for
...
Examples:
    ...
    $ KM-Security -site http://example.com/test/ 2>/dev/null

    $ KM-Security=4096 hulk -site http://example.com 2>/tmp/errlog
...
I am not related with original KM-Security utility in Python. Original KM-Security utility is authority of Barry Shteiman (http://sectorix.com). There are not any references to license in the original source then it is not under GPL. Ask author of the original utility about license. 
